...
function TryOnPanel({p,photo,looks,uploading,choose,selectLook,upload,close}:{p:Product|null;photo:string|null;looks:Look[];uploading:boolean;choose:(p:Product)=>void;selectLook:(look:Look)=>void;upload:(e:ChangeEvent<HTMLInputElement>)=>void;close:()=>void}){
 ...
 return <aside className="floating-studio">
   ...
   <div className="panel-stage">
    {!photo ? <div className="empty-model"><div className="upload-orb"><ImagePlus size={25}/></div><h4>Start with your photo</h4><p>Upload once, then keep browsing. Your photo stays here while you try different looks.</p><label className="upload-btn"><Upload size={15}/>{uploading?'Reading photo…':'Upload photo'}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={upload} /></label><span className="helper">Full-body photo works best · JPG, PNG or WebP</span></div> : <div className="model-preview"><img src={previewImage!} alt={p?`Virtual try-on preview for ${p.name}`:'Your uploaded model'}/><div className="preview-shade"/>{p&&<div className="look-chip"><Sparkles size={13}/><span><b>{p.name}</b><small>UI try-on preview</small></span></div>}</div>}
   </div>
